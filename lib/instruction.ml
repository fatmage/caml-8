...
let iSE_rb   = RegVal (fun s reg v -> if U8.eq (get_reg s reg) v then tick_pc s else s) (* 3xkk *)
let iSNE_rb  = RegVal (fun s reg v -> if U8.neq (get_reg s reg) v then tick_pc s else s) (* 4xkk *)
let iSE_rr   = DReg   (fun s r1 r2 -> if r1 == r2 then tick_pc s else s) (* 5xy0 *)
let iLD_rb   = RegVal (fun s reg v -> set_reg s reg v)  (* 6xkk *)
let iADD_rb  = RegVal (fun s reg v -> set_reg s reg (U8.add (get_reg s reg) v)) (* 7xkk *)
...
let iRND     = RegVal (fun s reg v -> s)  (* Cxkk *)
let iDRW     = DRegVal (fun s r1 r2 v -> s)    (* Dxyn *)
