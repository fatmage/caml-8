...
let iOR_rr   = DReg   (fun s r1 r2 -> print_string "8xy1\n"; tick_pc (set_reg (set_flag s U8.zero) r1 
                                                       (U8.logor (get_reg s r1) (get_reg s r2))))                       (* 8xy1 *)
let iAND_rr  = DReg   (fun s r1 r2 -> print_string "8xy2\n"; tick_pc (set_reg (set_flag s U8.zero) r1 
                                                       (U8.logand (get_reg s r1) (get_reg s r2))))                      (* 8xy2 *)
let iXOR_rr  = DReg   (fun s r1 r2 -> print_string "8xy3\n"; tick_pc (set_reg (set_flag s U8.zero) r1 (
                                                       (U8.logxor (get_reg s r1) (get_reg s r2)))))                     (* 8xy3 *)
let iADD_rr  = DReg   (fun s r1 r2 -> print_string "8xy4\n"; tick_pc (let res_int = (U8.to_int (get_reg s r1)) + (U8.to_int (get_reg s r2)) in 
...
let iSKNP    = Reg    (fun s reg -> print_string "ExA1\n"; match check_key s (get_reg s reg) with
...
let iADD_i   = Reg    (fun s reg -> print_string "Fx1E\n"; tick_pc (add_ir s (u8_to_16 (get_reg s reg))))                                      (* Fx1E *)
let iLD_font = Reg    (fun s reg -> print_string "Fx29\n"; tick_pc (set_ir s (U16.mul (u8_to_16 (get_reg s reg)) (U16.of_int 5))))             (* Fx29 *)
let iLD_mem  = Reg    (fun s reg -> print_string "Fx33\n"; let value = get_reg s reg in 
                                            ...
                                            let hund = U8.rem (U8.div value (U8.of_int 100)) ten in
                                              let addr = get_ir s in
                                                tick_pc (update_mem 
                                                          ...
                                                          (U16.succ addr) tens) 
                                                         addr hund))                                                    (* Fx33 *)
let iLD_memi = Reg    (fun s reg -> print_string "Fx55\n"; let rec helper = fun s ir curr max ->
